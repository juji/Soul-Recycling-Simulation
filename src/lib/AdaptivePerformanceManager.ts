...
export class AdaptivePerformanceManager {
  ...
  private adjustQualityForHardware(
    quality: Record<QualityLevel, QualitySettings>
  ): Record<QualityLevel, QualitySettings> {
    const multiplier = this.getHardwareMultiplier();
    ...
    Object.keys(quality).forEach(level => {
      ...
      quality[qualityLevel].maxConnectionChecks = Math.floor(
        quality[qualityLevel].maxConnectionChecks * multiplier
      );
    });
  ...
  private getHardwareMultiplier(): number {
    ...
    if (this.hardwareProfile.cpuCores >= 8) {
      multiplier *= 1.3;
    } else if (this.hardwareProfile.cpuCores >= 4) {
      multiplier *= 1.0;
    } else {
      multiplier *= 0.7;
    }

    if (this.hardwareProfile.memory.category === 'high') {
      multiplier *= 1.2;
    } else if (this.hardwareProfile.memory.category === 'low') {
      multiplier *= 0.8;
    }

    if (this.hardwareProfile.gpu.category === 'high') {
      multiplier *= 1.4;
    } else if (this.hardwareProfile.gpu.category === 'low') {
      multiplier *= 0.6;
    }

    if (this.hardwareProfile.deviceType === 'mobile') {
      multiplier *= 0.5;
    }
  ...
  public updateMetrics(
    ...
    Object.keys(this.metrics).forEach(key => {
      ...
      if (
        Array.isArray(this.metrics[metricKey]) &&
        (this.metrics[metricKey] as number[]).length > windowSize
      ) {
  ...
  private shouldAdapt(
    ...
    if (
      avgFPS > thresholds.headroom.fps &&
  ...
  private determineOptimalQuality(
    ...
    if (avgFPS < thresholds.critical.fps || memoryPressure > 0.95) {
      ...
      return Math.max(0, currentIndex - 1) >= 0
        ? qualityLevels[Math.max(0, currentIndex - 1)]
        : 'minimal';
      ...
      return Math.min(qualityLevels.length - 1, currentIndex + 1) < qualityLevels.length
        ? qualityLevels[Math.min(qualityLevels.length - 1, currentIndex + 1)]
        : 'ultra';
  ...
  private recordPerformanceData(soulCount: number, fps: number, quality: QualityLevel): void {
    ...
    this.performanceTracker.qualityBenchmarks
      .get(quality)!
      .push({ soulCount, fps, timestamp: Date.now() });
  ...
  private updatePerformanceTracking(metric: {
    fps: number;
    soulCount: number;
    memory: number;
    timestamp: number;
  ...
  private adaptQuality(
    ...
    metrics: AdaptationHistory['metrics']
  ): QualitySettings {
  ...
  private getAdaptationReason(metrics: AdaptationHistory['metrics']): string {
    if (metrics.avgFPS < 20) {
      return 'Critical FPS drop';
    }
    if (metrics.memoryPressure > 0.9) {
      return 'Memory pressure';
    }
    if (metrics.avgFPS < 30) {
      return 'Low FPS performance';
    }
    if (metrics.fpsStability < 0.6) {
      return 'FPS instability';
    }
    if (metrics.avgFPS > 55 && metrics.memoryPressure < 0.6) {
      return 'Performance headroom available';
    }
  ...
  private learnFromAdaptation(adaptation: AdaptationHistory): void {
    if (
      adaptation.from === 'high' &&
      adaptation.to === 'medium' &&
      adaptation.reason.includes('FPS')
    ) {
      this.hardwareProfile.performance = 'medium';
    } else if (
      adaptation.from === 'medium' &&
      adaptation.to === 'high' &&
      adaptation.metrics.avgFPS > 50
    ) {
  ...
  public getFPSStability(): number {
    ...
    const variance =
      this.metrics.fps.reduce((sum, fps) => sum + Math.pow(fps - avg, 2), 0) /
