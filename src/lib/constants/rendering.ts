...
export function validateCameraSettings(settings: any): settings is CameraSettings {
  return (
    ...
    typeof settings.FOV === 'number' && settings.FOV > 0 &&
    typeof settings.NEAR === 'number' && settings.NEAR > 0 &&
    typeof settings.FAR === 'number' && settings.FAR > settings.NEAR &&
...
export function validateLightingSettings(settings: any): settings is LightingSettings {
  return (
    ...
    settings.POINT_LIGHTS.every((light: any) => 
      validateLightSettings(light) && 
...
export function validateLightSettings(settings: any): settings is LightSettings {
  return (
    ...
    typeof settings.intensity === 'number' && settings.intensity >= 0
  );
}
...
export function validateGeometrySettings(settings: any): settings is GeometrySettings {
  return (
    typeof settings === 'object' &&
    typeof settings.HUMAN_RADIUS === 'number' && settings.HUMAN_RADIUS > 0 &&
    typeof settings.GPT_SIZE === 'number' && settings.GPT_SIZE > 0 &&
    typeof settings.DEWA_RADIUS === 'number' && settings.DEWA_RADIUS > 0 &&
...
function validateMaterialOpacity(settings: any): settings is MaterialOpacity {
  return (
    typeof settings === 'object' &&
    typeof settings.DEFAULT === 'number' && settings.DEFAULT >= 0 && settings.DEFAULT <= 1 &&
    typeof settings.DEWA === 'number' && settings.DEWA >= 0 && settings.DEWA <= 1
  );
}
...
export function validateControlsSettings(settings: any): settings is ControlsSettings {
  return (
    typeof settings === 'object' &&
    typeof settings.DAMPING_FACTOR === 'number' && settings.DAMPING_FACTOR >= 0 &&
    typeof settings.W_MAX === 'number' && settings.W_MAX > 0 &&
    typeof settings.ENABLE_PAN === 'boolean' &&
    typeof settings.ENABLE_ZOOM === 'boolean' &&
    typeof settings.ENABLE_ROTATE === 'boolean' &&
    typeof settings.MIN_DISTANCE === 'number' && settings.MIN_DISTANCE > 0 &&
    typeof settings.MAX_DISTANCE === 'number' && settings.MAX_DISTANCE > settings.MIN_DISTANCE
  );
}
