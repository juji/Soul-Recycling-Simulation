    ...
    if (type === 'init') {
        ...
        const updatedSoulsData = souls.map(soul => {
            ...
            souls.forEach(otherSoul => {
                ...
                // Gods also participate in separation to avoid clumping with others, and others separate from them
                const distanceToNeighbor = vec.distanceTo(soul.position, otherSoul.position);
            ...
            if (!soul.isGod) {
                ...
                if (soul.chosenGodId !== null) {
                    ...
                    if (currentlyChosenGod) {
                        ...
                        if (distanceToChosenGod < GOD_ATTRACTION_RADIUS) {
                            ...
                            soul.chosenGodId = null; // Chosen god is too far, reset
                        }
                    } else {
                        soul.chosenGodId = null; // Chosen god no longer exists
                    }
                }

                if (targetGod === null) { // If no chosen god or chosen god is out of range/gone
                    let closestGod = null;
                ...
                if (targetGod) {
                    ...
                    // Ensure attraction only happens if still within radius (should be true if chosen)
            ...
            // Slightly perturb the velocity - Gods are more deliberate, less random perturbation
            if (!soul.isGod) {
                soul.velocity.x += (Math.random() - 0.5) * 0.2;
                soul.velocity.y += (Math.random() - 0.5) * 0.2;
                soul.velocity.z += (Math.random() - 0.5) * 0.2;
            } else {
                soul.velocity.x += (Math.random() - 0.5) * 0.01; // Much smaller perturbation for gods
                soul.velocity.y += (Math.random() - 0.5) * 0.01;
                soul.velocity.z += (Math.random() - 0.5) * 0.01;
            }

            // Pointer interaction logic
            if (pointerPosition3D && soul.isHuman && !soul.isGod) { // Gods are not affected by pointer
            ...
            return {
                ...
                newHSL: { 
                    h: soul.baseHSL.h, 
                    s: soul.baseHSL.s, 
                    l: soul.isGod ? soul.baseHSL.l : newLightness // God souls retain base lightness, others pulse
                },
